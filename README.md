...
1. Предложение для покупателей (Buyer Incentives):
    ...
    - Формула кэшбэка может быть плавающей: чем выше уровень лояльности (или объем покупок пользователя), тем больший
      процент он получает.
    - Пример формулы: $Reward_tokens = purchase_amount * R(tier)$, где $R(tier)$ – коэффициент кэшбэка для уровня
...
2. Предложение для продавцов (Seller Incentives):
...
3. Предложение для логистических партнёров (Logistics Incentives):
...
4. Предложение для рефералов и маркетинга (Referral & Marketing):
