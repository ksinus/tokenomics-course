...
Далее рассматривается механизм покупки токенов непосредственно у контракта. Покупка на bonding curve не сводится к простому умножению «текущей цены» на «количество», поскольку цена возрастает в рамках одной операции. Следовательно, корректная стоимость покупки \( \Delta S \) токенов при текущем предложении \( S \) соответствует площади под кривой на отрезке от \( S \) до \( S + \Delta S \). Это записывается в виде интеграла, который затем раскрывается в явную формулу для ясности вычислений:
\[
\mathrm{Cost}_{\text{buy}}(S,\Delta S) = \int_{S}^{S+\Delta S} P(x) \, dx
= a\Delta S + \frac{b}{2} \left( (S + \Delta S)^2 - S^2 \right).
\]
...
Чтобы у токена была двусторонняя ликвидность, я фиксирую резерв как площадь под ценовой кривой от нуля до текущего предложения, потому что это напрямую связывает параметры кривой с тем, какой объём резерва должен поддерживать обратный выкуп:
...
\mathrm{Payout}_{\text{sell}}(S,\Delta S)=\int_{S-\Delta S}^{S}P(x)\,dx
= a\Delta S+\frac{b}{2}\left(S^2-(S-\Delta S)^2\right).
Чтобы снизить арбитражный шум и риск “bank run”, на выкуп вводится комиссия f_{\text{sell}}\in[0,1):
\mathrm{Payout}^{\text{net}}_{\text{sell}}(S,\Delta S)=(1-f_{\text{sell}})\cdot \mathrm{Payout}_{\text{sell}}(S,\Delta S).

Коэффициенты a и b я не выбираю “на глаз”, а привязываю к контролируемым целям. Если задан ценовой коридор (P_0,P_{\max}) и cap \,S_{\max}, то из условий P(0)=P_0 и P(S_{\max})=P_{\max} следует:
a=P_0,\qquad b=\frac{P_{\max}-P_0}{S_{\max}}.
Если задан целевой сбор резерва C при достижении \,S_{\max}, то:
C=R(S_{\max})=aS_{\max}+\frac{b}{2}S_{\max}^2.
При фиксированных P_0 и P_{\max} удобная форма сразу даёт связь “средняя цена \times объём”:
C=S_{\max}\cdot\frac{P_0+P_{\max}}{2},
\qquad
S_{\max}=\frac{2C}{P_0+P_{\max}},
\qquad
b=\frac{P_{\max}-P_0}{S_{\max}}.

Отдельно я страхуюсь от “кита”: при типичном крупном ордере \Delta S_{\text{whale}} ограничиваю допустимое изменение мгновенной цены \Delta P. Для линейной кривой:
\Delta P=P(S+\Delta S_{\text{whale}})-P(S)=b\Delta S_{\text{whale}}
\quad\Rightarrow\quad
b\le \frac{\Delta P}{\Delta S_{\text{whale}}}.

Числовая sanity-check проверка (чтобы не ошибиться порядками): при \,S_{\max}=500\,000\,000, \,P_0=0.05, \,P_{\max}=1.05 получаем
a=0.05,\qquad b=\frac{1.05-0.05}{500\,000\,000}=2\cdot 10^{-9},
\qquad
C=S_{\max}\cdot\frac{P_0+P_{\max}}{2}=275\,000\,000.
Эта тройка (a,b,R) и есть базовая гарантия: кривая задаёт цену, резерв предсказывает объём поддержки выкупа, а комиссия на sell добавляет устойчивость.
