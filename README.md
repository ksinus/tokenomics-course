...
## Базовое описание формулы

При фиксации bonding curve для WBX отправной точкой является формализация двух обязательных элементов, без которых невозможно определение ни цены, ни ликвидности: текущего объёма предложения токена, уже прошедшего через кривую, и правила, по которому контракт преобразует это предложение в цену. Текущее выпущенное количество обозначается как $S \ge 0$, а цена одного токена в резервном активе — как $P(S)$. В минималистичной версии, удобной для объяснения и верификации в рамках технической документации, выбирается линейная кривая благодаря её прозрачности и предсказуемой траектории роста цены. Формально она задаётся уравнением:

$$P(S) = a + bS, \quad a > 0, \; b > 0.$$

Здесь $a$ представляет стартовую цену при нулевом выпуске через кривую, а $b$ — наклон кривой, определяющий скорость роста цены по мере увеличения $S$. На данном этапе важно различать два различных понятия «цены»: на графике отображается мгновенная цена покупки одной маржинальной единицы токена, а не средняя цена приобретения пакета.

Далее рассматривается механизм покупки токенов непосредственно у контракта:

$$\mathrm{Cost}\_{\text{buy}}(S,\Delta S) = \int_{S}^{S+\Delta S} P(x) \, dx= a\Delta S + \frac{b}{2} \left( (S + \Delta S)^2 - S^2 \right).$$

Данная запись иллюстрирует ключевую идею: крупный покупатель не может приобрести значительный объём по первоначальной низкой цене, поскольку в процессе покупки он самостоятельно перемещает цену вверх на величину, заложенную в интеграле.

Чтобы у токена была двусторонняя ликвидность, фиксируем резерв как площадь под ценовой кривой от нуля до текущего предложения, потому что это напрямую связывает параметры кривой с тем, какой объём резерва должен поддерживать обратный выкуп:

$$R(S)=\int_{0}^{S}P(x)\,dx=aS+\frac{b}{2}S^2.$$

Выкуп симметричен покупке: если пользователь продаёт $\Delta S$, то контракт сжигает токены, а выплата — площадь под кривой на отрезке $[S-\Delta S,\,S]$:

$$\mathrm{Payout}_{\text{sell}}(S,\Delta S)=\int_{S-\Delta S}^{S}P(x)\,dx = a\Delta S+\frac{b}{2}\left(S^2-(S-\Delta S)^2\right).$$

Чтобы снизить арбитражный шум, на выкуп вводится комиссия $f_{\text{sell}}\in[0,1)$:

$$\mathrm{Payout}^{\text{net}}_{\text{sell}}(S,\Delta S)=(1-f_{\text{sell}})\cdot \mathrm{Payout}_{\text{sell}}(S,\Delta S).$$

Если задан ценовой коридор $(P\_0,P\_{\max})$ и $cap, S\_{\max}$, то из условий $P(0)=P_0$ и $P(S\_{\max})=P\_{\max}$ следует:

$$a=P_0,\qquad b=\frac{P_{\max}-P_0}{S_{\max}}.$$
...
Формализую подход с верхнеуровневой аллокацией, а внутри которой есть своя механика: существует общий лимит предложения $$S^{(g)}\_{max}$$, а внутри него выделены корзины $$S^{(g)}_{max}$$ по группам, причём сумма корзин не превышает общего лимита. На уровне времени это означает, что выдача группе ещё ограничена остатком корзины: 

$$D^{(g)}_t \le S^{(g)}_{\max} - S^{(g)}_t, \qquad \sum_g S^{(g)}_{\max} \le S_{\text{total}}.$$

Тогда покупатели могут жить в своей cashback-корзине и использовать внутри неё bonding curve-логику расчёта объёма, продавцы могут жить в своей halving-корзине и следовать bitcoin-like уменьшению наград по эпохам, логистика — в своей корзине SLA-бонусов, но вся система остаётся непротиворечивой, потому что каждая группа физически не может превысить свой $S^{(g)}\_{\max}$, а суммарно все корзины не могут превысить $S\_{\text{total}}$. В этом варианте bonding curve может быть использована локально как «внутренняя цена/норма выдачи» внутри конкретной корзины, не ломая общую эмиссию.
...
а в относительных терминах по обращению $S\_{\text{circ}}$ она равна:

$$\mathrm{Infl}_{\text{net}}\approx \frac{\mathrm{Mint}_{\text{year}}-\mathrm{Burn}_{\text{year}}-\mathrm{UtilBurn}_{\text{year}}}{S_{\text{circ}}}.$$

# Споры и Абьюз платформы

В WBX споры неизбежны между покупателем и продавцом (качество/возвраты), покупателем и доставкой (утеря/повреждение), продавцом и платформой (блокировки/выплаты), продавцом и логистикой (потери/сроки), курьером и платформой (штрафы). Ключевая цель токеномики арбитража — сделать ложные претензии и “оспаривание ради затяжки” экономически невыгодными, а честное участие валидаторов — выгодным.

Базовый контур — оптимистичный оракул UMA-стиля: любое утверждение считается истинным, пока его не оспорили в окно. Инициатор и оппонент кладут залоги в WBX; если оспаривания нет, решение исполняется автоматически, залог возвращается. Если оспаривание есть, спор уходит на голосование валидаторов, где у каждого валидатора есть собственный stake, который может быть частично “срезан” за голос против финального решения. Требуемый залог привязывается к сумме спора и репутации, чтобы крупные споры были дорогими для абьюза, а проверенным участникам не приходилось “замораживать” слишком много. Например:

$$\mathrm{Stake}_{\text{assert}} = \kappa \cdot S \cdot g(\mathrm{rep}), \qquad \mathrm{Stake}_{\text{chall}}=\lambda\cdot \mathrm{Stake}_{\text{assert}},\ \lambda\ge 1.$$

Дальше штраф лжи обеспечивается перераспределением залогов: проигравшая сторона теряет часть залога, который уходит победителю, валидаторам и/или в burn/казну, так что ожидаемая стоимость мошенничества становится положительной. В простейшем виде:

$$
\mathrm{Penalty}=\phi\cdot \mathrm{Stake}_{\text{loser}},\quad \phi\in(0,1],
\qquad
\mathrm{Penalty}=\mathrm{ToWinner}+\mathrm{ToValidators}+\mathrm{Burn}.
$$

Вознаграждение валидаторов можно делать из штрафного пула, распределяя его только тем, кто совпал с итогом:

$$
\mathrm{Reward}_i=\frac{\mathrm{ToValidators}}{N_{\text{correct}}},
\qquad
\mathrm{Slash}_i=\psi\cdot \mathrm{Stake}^{\text{juror}}_i \ \text{для “неправых”},\ \psi\in(0,1).
$$

Этого достаточно, чтобы честность стала доминирующей стратегией при условии, что штрафы и залоги заметны относительно потенциальной выгоды от обмана.

Для тяжёлых кейсов (платформа vs продавец, большие суммы, прецеденты) полезен второй режим — прямой суд валидаторов: спор сразу уходит в жюри из случайно выбранных валидаторов, чтобы снизить риск сговора и дать легитимность. Вес голосов можно держать равным или использовать сублинейное взвешивание, но в любом случае критичны две защиты: случайный отбор и commit–reveal, чтобы голоса нельзя было подстроить.

Роль WBX здесь сводится к четырём функциям: залоги сторон (skin in the game), stake валидаторов (гарантия ответственности), источник наград (из штрафов/комиссий), и механизм наказания (slash/burn), который делает абьюз дорогостоящим и тем самым удерживает систему в равновесии.
